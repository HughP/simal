...
public class Project implements IProject {
	...
	/**
	 * Returns the first name in the set of names contained
	 * within the Elmo Project class.
	 */
	...
	/**
	 * Sets the name of the project. Note that this assumes that
	 * there is only one name. In the event of there being more 
	 * than one name it throws an exception. 
	 * 
	 * @throws ProjectException if the project has more than one name
	 */
	public void setName(String name) throws ProjectException {
		if (elmoProject.getDoapNames() != null) {
			if (elmoProject.getDoapNames().size() != 1) {
				throw new ProjectException("At present the Project wrapper can only handle a single project name.");
			}
