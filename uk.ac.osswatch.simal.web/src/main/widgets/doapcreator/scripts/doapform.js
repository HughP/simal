...
function generatePersonHtml(personIndex) {
  ...
  personHtml +="				</select>\n";
  personHtml +="			<input type=\"text\" id=\"person_name_" + personIndex + "\" class=\"short\"/>\n";
  personHtml +="			<input type=\"text\" id=\"person_email_" + personIndex + "\" class=\"short\"/>\n";
  personHtml +="			<input type=\"text\" id=\"person_url_" + personIndex + "\" class=\"short\"/>\n";
  personHtml +="			<input type=\"text\" id=\"person_pic_" + personIndex + "\" class=\"short\"/><br />\n";
  return personHtml;
}
...
function processPerson(personIndex) {
  if (!exists(document.getElementById("person_name_" + personIndex))) return "";
  ...
  if (person_name == "" || person_email == "") return "";
  ...
  peopleDoap += "   <foaf:mbox_sha1sum>" + hex_sha1(person_email) + "</foaf:mbox_sha1sum>\n";
...
function allMandatoryFieldsOK() {
  var allMandatoryFieldIds = [ "name", "shortname", "shortdesc", "description", "blog", "homepage",
		"blogfeed", "projectemail", "person_email_1", "person_name_1", "person_pic_1", "lead_institution", 
		"department", "dept_postcode", "resp_reports_name", "resp_reports_email", "licence", 
		"cont_licence", "jiscmanager"
		];
  var  allNotNoneFields = ["primary_product", "secondary_product", "primary_audience"];

  for (i=0;i<allMandatoryFieldIds.length;i++) {
    if (!exists(document.getElementById(allMandatoryFieldIds[i]))) alert("What do you mean, " + allMandatoryFieldIds[i]);
  ...
  if(document.getElementById("resp_reports_phone").value == "" && document.getElementById("resp_reports_skype").value == "") {
...
function checkUrl(fieldName) {
  ...
  if(fieldValue != "") {
    ...
    if (fieldValue.indexOf("http://") != 0 && fieldValue.indexOf("https://") != 0) {
...
function generate() {
  ...
  if(!allMandatoryFieldsOK()) return false;
  ...
  try {
	...
	if (blogfeed != "") {
      doap += " <foaf:seeAlso dc:title=\"Project blog\" rdf:resource=\"" + blogfeed + "\" dc:format=\"application/rss+xml\" />\n";
	}
	
	doap += " <simal:reporter>\n";
    doap += "  <foaf:Person rdf:about=\"" + PEOPLE_NAMESPACE_URL + hex_sha1(resp_reports_email) + "\">\n";
    doap += "   <foaf:name>" + resp_reports_name + "</foaf:name>\n";
    doap += "   <foaf:mbox rdf:resource=\"mailto:" + resp_reports_email + "\" />\n";
    doap += "   <foaf:mbox_sha1sum>" + hex_sha1(resp_reports_email) + "</foaf:mbox_sha1sum>\n";
	if (resp_reports_phone != "") {
      doap += "   <foaf:phone>" + resp_reports_phone + "</foaf:phone>\n";
	}
	if (resp_reports_skype != "") {
      doap += "   <foaf:skypeID>" + resp_reports_skype + "</foaf:skypeID>\n";
	}
    doap += "  </foaf:Person>\n";
	doap += " </simal:reporter>\n";

	doap += " <simal:reporter>\n";
    doap += "  <foaf:Person rdfs:label=\"Project account\" rdf:about=\"" + PEOPLE_NAMESPACE_URL + hex_sha1(projectemail) + "\">\n";
    doap += "   <foaf:mbox rdf:resource=\"mailto:" + projectemail + "\" />\n";
    doap += "  </foaf:Person>\n";
	doap += " </simal:reporter>\n";
	...
	if (jiscmanager != "") {
	  ...
	  doap += " </foaf:fundedBy>\n";
      doap += " <simal:fundingManager>" + jiscmanager + "</simal:fundingManager>\n";
	}
	
    if (lead_institution != "") {
	  doap += "<vendor><foaf:Organization dc:title=\"Lead institution\"><foaf:name>" + lead_institution + "</foaf:name>";
      if (department != "") {
        doap += "<foaf:member><foaf:Organization dc:title=\"Department\"><foaf:name>" + department + "</foaf:name>\n";
        if (dept_postcode != "") {
          doap += "<foaf:based_near><v:Address><v:postal-code>" + dept_postcode + "</v:postal-code></v:Address></foaf:based_near>";
	    }
	    doap += "</foaf:Organization></foaf:member>\n";
      }
	  
      if (project_partners != "") {
        var partnersArray = project_partners.split(",");
        for (var i = 0; i < partnersArray.length; i++) {
            doap += " <simal:projectPartner><foaf:Organization><foaf:name>" + partnersArray[i].trim();
            doap += "</foaf:name></foaf:Organization></simal:projectPartner>\n";
        }
      }
	  doap += "</foaf:Organization></vendor>\n";
	}
	...
	if (licence != "") { 
      doap += " <license rdf:resource=\"http://usefulinc.com/doap/licenses/" + licence + "\"/>\n";
	}
	
	if (cont_licence != "") { 
      doap += " <license rdf:resource=\"http://usefulinc.com/doap/licenses/" + cont_licence + "\"/>\n";
	}
	
	if (primary_product != "none") {
      doap += " <category rdf:resource=\"" + CATEGORY_NAMESPACE_URL;
	  doap += primary_product.split(' ').join('').toLowerCase() + "\" rdfs:label=\"" + primary_product + "\" />\n";
	}
	
	if (secondary_product != "none") {
      doap += " <category rdf:resource=\"" + CATEGORY_NAMESPACE_URL;
      doap += secondary_product.split(' ').join('').toLowerCase() + "\" rdfs:label=\"" + secondary_product + "\" />\n";
	}
	
	if ( primary_audience != "") {
      doap += " <audience>" + primary_audience + "</audience>\n";
	}

	if ( secondary_audience != "none") {
      doap += " <audience>" + secondary_audience + "</audience>\n";
	}
    ...
    if (langs != "") {
        ...
        for (var i = 0; i < langsArray.length; i++) {
            doap += " <programming-language>" + langsArray[i].trim() + "</programming-language>\n";
        }
    }
...
function commitDoapFile(doap){
	...
	xml_request.onreadystatechange = function()
	{
		if(xml_request.readyState == 4 && xml_request.status == 200){
		    if(xml_request.responseText.indexOf("null") == 0) {
			  alert('Project successfully saved to the registry.\nPlease keep the contents for your record.');
			} else {
